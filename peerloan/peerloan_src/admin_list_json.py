...
class bor_list_json(APIView):
    ...
    def get(self, request, format=None):
        ...
        if data_src == 'fund_matching_bor':
            bor_list = [bor for bor in bor_list if
                        bor.status in ['VALIDATED', 'FUND MATCHING', 'FUND MATCHING COMPLETED']]
            if prod_id != 'all':
            ...
            return Response(content)
        if cate == 'application':
            apply_state_list = ['AUTO APPROVED', 'DOC UPLOADED', 'AGREEMENT CONFIRMED', 'PENDING DOC/IV', 'VALIDATED',
                                'CANCELLED', 'REJECTED']
        ...
        if data_src == 'fund_matching_inv':
            ...
            for inv in inv_list:
                ...
                data_list.append({
                    ...
                    'investment_start_date': datetime.strftime(timezone.localtime(inv.create_timestamp),
                                                               '%Y/%m/%d %H:%M'),
...
class repay_list_json(APIView):
    ...
    def get(self, request, format=None):
        ...
        if due_on != 'All':
            ...
            bor_list = [bor for bor in bor_list if
                        datetime.strptime(timezone.localtime(bor.create_timestamp).strftime('%m/%d/%Y'),
                                          '%m/%d/%Y') >= due_on]
    ...
    def get(self, request, format=None):
        ...
        for usr in usr_list:
            ...
            try:
                ...
                name = '--'
            allocated_amount = sum([inv.usable_amount + inv.on_hold_amount for inv in inv_list]) + sum(
                [loan.remain_principal for loan in loan_list])

            try:
                fund_matching_percentage = round((sum([inv.on_hold_amount for inv in inv_list]) + sum(
                    [loan.remain_principal for loan in loan_list])) / allocated_amount, 2)
            except ZeroDivisionError:
                fund_matching_percentage = 0
            try:
                delinquent_percentage = round(
                    sum([loan.remain_principal for loan in loan_list if 'OVERDUE' in loan.status]) / sum(
                        [loan.remain_principal for loan in loan_list]), 2)
            except ZeroDivisionError:
...
class aut_list_json(APIView):
    ...
    def get(self, request, format=None):
        ...
        if range == 'bor_only':
            ...
            aut_list = [aut for aut in aut_list if
                        User.objects.get(id=aut.usr_id).type == request.session['handling_side']]
            for aut in aut_list:
...
class ph_list_json(APIView):
    ...
    def get(self, request, format=None):
        ...
        for bor in bor_list:
            data_list.append(
                {
                    "due_date": bor.expected_end_date,
                    "installment_amount": bor.instalment_lender,
                    "loan_No": bor.ref_num,
                    "lender_No": bor.usr_id,
                    "tenor": bor.repaid_month,
                    "loan_state": bor.status,
                    "disbursement_date": bor.create_timestamp,
                    "pay_or_not_pay": '--',
                }
            )
