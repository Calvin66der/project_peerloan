...
def application(request):
	if request.session['handling_side'] == 'B':
		...
		if 'detail' in request.META.get('PATH_INFO').split('/'):
			...
			date_of_birth = pytz.timezone('Asia/Hong_Kong').localize(datetime.strptime(bor_detail['Date of Birth'], '%Y-%m-%d'))
			...
			'home_status_list': ['Public Housing', 'Owned by Family', 'Rent', 'Quarter', 'Student Hall of Residence'],
			'living_with': sup_fn.try_KeyError(bor_detail, 'Living with'),
			'living_with_list': ['Parents', 'Relatives', 'Friends or Classmates', 'Others'],
			'school': sup_fn.try_KeyError(bor_detail, 'Studying University'),
			'subject': sup_fn.try_KeyError(bor_detail, 'Subject'),
			'subject_list': ['Medical/Health','Law','Accounting','Construction and Environment','Engineering','Design',
							'Business/Finance/Economic','Education and Language','Information Technology/Computing',
			...
			code, network_score_detail = fs.get(endpoint='users/partner-id/'+str(bor.usr_id)+'/show/social-network-data')
			...
			social_score = {
				...
				'friendlyscore_href': 'https://friendlyscore.com/company/user/application/user/show/'+sup_fn.try_KeyError(main_score_detail, 'id')
			}
		...
		if 'all_application' in request.META.get('PATH_INFO').split('/'):
			...
			status_list = ['AUTO APPROVED', 'DOC UPLOADED', 'AGREEMENT CONFIRMED', 'PENDING DOC/IV',
			 'VALIDATED', 'CANCELLED', 'REJECTED', 'FUND MATCHING', 'FUND MATCHING COMPLETED', 'DISBURSED',
	...
	if request.session['handling_side'] == 'L':
		...
		if 'detail' in request.META.get('PATH_INFO').split('/'):
			...
			try:
				...
				age = int((timezone.localtime(timezone.now()) - pytz.timezone('Asia/Hong_Kong').localize(datetime.strptime(uor_details['Individual']['Date of Birth'], '%Y-%m-%d'))).days / float(365))
				occupation = uor_details['Individual']['Occupation']
			else:
				# is corporate
				account_type = 'Corporate'
				name = uor_details['Corporate']['Company Name']
				age = int((timezone.localtime(timezone.now()) - pytz.timezone('Asia/Hong_Kong').localize(datetime.strptime(uor_details['Corporate']['Established at'], '%Y-%m-%d'))).days / float(365))
			...
			'type_of_employment_list': ['Full Time Employed', 'Part Time Employed', 'Self-Employed', 'Unemployed', 'Housewife', 'Retired'],
...
def repayment(request):
	...
	if 'detail' in request.META.get('PATH_INFO').split('/'):
		...
		for los in los_list:
			...
			'due_date': los.due_date,
			'overdue_day': los.overdue_days,
			'overdue_interest': str(round(los.overdue_interest_accumulated, 2))+'<br>(paid:'+str(round(los.paid_overdue_interest, 2))+')',
			'late_charge': str(round(los.late_charge, 2))+'<br>(paid:'+str(round(los.paid_late_charge, 2))+')',
			'total': str(round(los.instalment + los.overdue_interest_accumulated + los.late_charge, 2))
			+'<br>(paid:'+str(round(los.paid_principal + los.paid_interest + los.paid_late_charge + los.paid_overdue_interest, 2))+')',
			}
		...
		early_settlement_amount = outputs['early_settlement_amount']+outputs['late_charge']-outputs['overpay_amount']
	...
	if request.session['handling_side'] == 'L':
		if 'ledger' in request.META.get('PATH_INFO').split('/'):
			...
			for inv in inv_list:
				...
				loans = [loan for loan in loans if 'MATCHED' not in loan.status and 'COMPLETED' not in loan.status and 'RELEASE' not in loan.status]
				for loan in loans:
					loan_list.append(loan)
			
			total_amount = sum([loan.remain_principal_lender for loan in loan_list])
			try:
				write_off_percentage = sum([loan.remain_principal_lender for loan in loan_list if 'WRITE OFF' in loan.status]) / sum([loan.remain_principal_lender for loan in loan_list])
			...
			infos = {
				...
				'adjusted_return_percentage': str(round((weighted_interest_rate - write_off_percentage)*100,2))+'%',
			}
			content = {
				'cate': 'account',
				'sub_cate': '',
